...
const Contact = () => {
  ...
  const handleSubmit = (e) => {
    e.preventDefault();
    if(form.current.name.value === "" || form.current.message.value === "" || form.current.email.value === "" || form.current.subject.value === ""){
      return Swal.fire({
    ...
    emailjs.sendForm(process.env.REACT_APP_SERVICE_KEY, process.env.REACT_APP_TEMPLATE_KEY, form.current, process.env.REACT_APP_PUBLIC_KEY)
      .then((result) => {
        ...
        Swal.fire({
          html: "Woohoo! Your email just did a little hop, skip, and jump into my inbox! I'll jump right on it and get back to you soon!",
          width: 600,
          padding: "3em",
          color: "rgb(221 106 158)",
          background: "#fff ",
          backdrop: `
            rgb(34 68 150 / 19%)
            url("/cat-love.gif")
